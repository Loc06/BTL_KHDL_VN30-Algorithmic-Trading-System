...
from config import DATA_FULL_PATH, KETQUA_EMA_PATH, PREDICTIONS_PATH, ensure_dirs
...
# 1. ĐỌC FILE
# ════════════════════════════════════════════════════════════════
for fname in [KETQUA_EMA_PATH, DATA_FULL_PATH]:
    if not os.path.exists(fname):
        raise FileNotFoundError(f"Không tìm thấy {fname}!")

# --- ketqua.csv ---
kq = pd.read_csv(KETQUA_EMA_PATH, encoding='utf-8-sig')
...
ensure_dirs()
df_res.to_csv(PREDICTIONS_PATH, index=False, encoding='utf-8-sig')
...
print(f"✅ Đã lưu báo cáo: {PREDICTIONS_PATH}")
print("=" * 95)